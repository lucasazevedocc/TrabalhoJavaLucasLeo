...
public class Aluno implements PadraoDAO{
    ...
    public String gerarString(){
        ...
        if(this.cursando != null && !this.cursando.isEmpty()){
            string += ("Cursos:");
            string = this.cursando.keySet().stream().map((key) -> this.cursando.get(key)).map((curso) -> "\n               "+curso.getNomeDoCurso()).reduce(string, String::concat);
        }else{
