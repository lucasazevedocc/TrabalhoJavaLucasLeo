...
public class PanelAdicionarAluno extends JPanel implements PadraoPanel,ActionListener{
    ...
    private boolean alunoIsValid(){
        return this.aluno != null && this.aluno.getNome() != null && !this.aluno.getNome().isEmpty() && this.aluno.getEndereco() != null && !this.aluno.getEndereco().isEmpty()
                && this.aluno.getCursando() != null && !this.aluno.getCursando().isEmpty();
    }
    ...
    private void gerarCursos(){  
        ...
        if(this.cursosDisponiveis == null || this.cursosDisponiveis.isEmpty()){
            liberarFormulario(false,"Cadastre os cursos primeiro.");
        }else{
            this.cursosDisponiveis.stream().map((Object c) -> {
                JCheckBox checkBox = new JCheckBox("código do curso: "+((Curso) c).getCodCurso()+"   --   nome: "+((Curso) c).getNomeDoCurso());
                checkBox.addActionListener((ActionEvent ae) -> {
                    if(((JCheckBox) ae.getSource()).isSelected()){
                        aluno.getCursando().put(((Curso) c).getCodCurso(), (Curso) c);
                    }else{
                        aluno.getCursando().remove(((Curso) c).getCodCurso());
                    }
                });
                return checkBox;
